<?php 
  ...
  function waiterSaleGrandTotal($time) {
    global $sqlconnection;
    $total = 0;
    ...
    if ($time == "ALLTIME") {
      $query = "
          ...
          FROM tbl_order WHERE status ='Completed' AND username='{$_SESSION['username']}'
          ";
    }
      ...
      $query = "
          SELECT SUM(total) as grandtotal
          FROM tbl_order WHERE status ='Completed' AND username='{$_SESSION['username']}' AND order_date > DATE_SUB(NOW(), INTERVAL 1 ".$time.")
          ";
    }
...
<h2 class="text-center text-dark">Here observe your all activities</h2>
              ...
              <br/>
              <?php  $sql=" SELECT status FROM tbl_order  WHERE username='{$_SESSION['username']}' AND status ='Completed'";
         ...
         <label style="background-color: pink;margin-left: 15%" class="a"><b><?php echo $count; ?></b><br>Delivered Order</label>
          ...
          <?php  $sql=" SELECT status FROM tbl_order  WHERE username='{$_SESSION['username']}' AND status ='cancelled'";
              $res = mysqli_query($sqlconnection, $sql);
              $count = mysqli_num_rows($res);
              ?>
         <label style="background-color: #dac292;" class="a"><b><?php echo $count; ?></b><br>Cancelled Order</label>
          ...
          <?php
             $q = 
        "SELECT order_date FROM tbl_order WHERE status= 'Completed' AND username='{$_SESSION['username']}'  GROUP BY order_date HAVING COUNT(status) >2 ";
       ...
       <label style="background-color: #e3eaa7;" class="a"><b><?php echo waiterSaleGrandTotal("DAY"); ?>.00 Tk</b><br> Today's Sale</label>

           <label style="background-color: lightgreen;" class="a"><b><?php echo waiterSaleGrandTotal("WEEK"); ?>.00 Tk</b><br>Sales Per-Week</label>

           <label style="background-color: skyblue;" class="a"><b><?php echo waiterSaleGrandTotal("MONTH"); ?>.00 Tk</b><br>Sales Per-Month</label>

           <label style="background-color: lightyellow;" class="a"><b><?php echo waiterSaleGrandTotal("ALLTIME"); ?>.00 Tk</b><br>Sales In-Total</label>
 ...
 <footer class="sticky-footer">
	...
	<script type="text/javascript">
        ...
        function refreshTableOrder() {
         $( "#tblBodyCurrentOrder" ).load( "w_displayorder.php?cmd=currentorder" );
            }
