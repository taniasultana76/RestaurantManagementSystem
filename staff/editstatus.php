	...
	if (isset($_POST['status']) && isset($_POST['orderID'])) {
		...
		$addOrderQuery = "UPDATE tbl_order SET status = '{$status}', chef ='{$_SESSION['username']}',time = CURTIME() WHERE orderID = {$orderID};";
	...
	if (isset($_GET['orderID'])) {
		...
		$addOrderQuery = "UPDATE tbl_order SET status = '{$status}',time = CURTIME() WHERE orderID = {$orderID};";
