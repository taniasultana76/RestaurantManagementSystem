<?php
	...
	function getSalesGrandTotal($duration) {
		...
		if ($duration == "ALLTIME") {
			...
			$query = "
					SELECT SUM(total) as grandtotal
					FROM tbl_order WHERE status ='Completed' AND order_date > DATE_SUB(NOW(), INTERVAL 1 ".$duration.")
					";
		}
	...
	function getOnlineSalesGrandTotal($duration) {
		global $sqlconnection;
		$total = 0;
		...
		if ($duration == "ALLTIME") {
			...
			$query = "
					...
					WHERE status ='Delivered'AND date > DATE_SUB(NOW(), INTERVAL 1 ".$duration.")
					";
		}
